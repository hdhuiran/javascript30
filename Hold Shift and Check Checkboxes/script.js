...
function handleCheck(e) {
  ...
  if (e.shiftKey && e.target.checked) {
    ...
    checkboxes.map((element) => {
      if (
        (element === self || element === checkEle) && checkEle && checkEle !== self
      ) {
        isBetween = !isBetween;
      }
      if (isBetween) {
...
function extenCheck(e){
    ...
    const nearMaxCheckboxIndex = checkboxex_2.findIndex(
        (element,index) =>{
            return index !== nowSelIndex && index > nowSelIndex && element.checked === true;
        }
    )
    // console.log(nearMaxCheckboxIndex);

    const nearMinCheckboxIndex = checkboxex_2.findIndex(
        (element,index) =>{
            return index !== nowSelIndex && index < nowSelIndex && element.checked === true;
        }
    )
